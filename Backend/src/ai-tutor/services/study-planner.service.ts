...
export class StudyPlannerService {
  async generateStudyPlan(student: StudentProfile): Promise<string[]> {
    ...
    console.log(`StudyPlannerService: Generating study plan for user ${student.userId}`);
    ...
    if (student.mastery < 40) {
      ...
      student.strongTopics.forEach(topic => studyPlan.push(`Advanced practice for ${topic}`));
    }
