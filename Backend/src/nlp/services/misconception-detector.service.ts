...
export class MisconceptionDetectorService {
  ...
  private countMatches(
    ...
    const text = processed.cleanedText.toLowerCase();
  ...
  private buildBreakdown(processed: ProcessedText): MisconceptionBreakdown {
    return {
      conceptGapScore: this.countMatches(
        processed,

        MISCONCEPTION_KEYWORDS[MisconceptionType.ConceptGap],
      ),

      conceptConfusionScore: this.countMatches(
        processed,

        MISCONCEPTION_KEYWORDS[MisconceptionType.ConceptConfusion],
      ),

      procedureErrorScore: this.countMatches(
        processed,

        MISCONCEPTION_KEYWORDS[MisconceptionType.ProcedureError],
      ),

      calculationErrorScore: this.countMatches(
        processed,

        MISCONCEPTION_KEYWORDS[MisconceptionType.CalculationError],
      ),

      repeatedMistakeScore: this.countMatches(
        processed,

        MISCONCEPTION_KEYWORDS[MisconceptionType.RepeatedMistake],
      ),
    };
  }
  ...
  private determineType(breakdown: MisconceptionBreakdown): MisconceptionType {
    const scores = [
      {
  ...
  private confidence(breakdown: MisconceptionBreakdown): number {
    const total =
  ...
  private matchedKeywords(processed: ProcessedText): string[] {
    ...
    for (const keywords of Object.values(MISCONCEPTION_KEYWORDS)) {
      for (const keyword of keywords) {
        if (
          TextMatcher.matchesPhrase(
            text,
  ...
  private buildReasoning(
    ...
    if (type === MisconceptionType.None) {
      reasoning.push('No misconception indicators detected.');

      return reasoning;
    }

    reasoning.push(`Detected misconception type "${type}".`);
  ...
  detect(processed: ProcessedText): MisconceptionResult {
    const breakdown = this.buildBreakdown(processed);

    const type = this.determineType(breakdown);
